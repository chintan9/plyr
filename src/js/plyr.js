...
class Plyr {
  constructor(target, options) {
    ...
    // Setup interface
    // If embed but not fully supported, build interface now to avoid flash of
    // controls
    ...
    // Seek time will be recorded (in listeners.js) so we can prevent hiding
    // controls for a few seconds after seek
    this.lastSeekTime = 0;
  ...
  set loop(input) {
          ...
          /* const type = ['start', 'end', 'all', 'none',
             'toggle'].includes(input) ? input : 'toggle';
              ...
              switch (type) {
                  case 'start':
                      if (this.config.loop.end && this.config.loop.end <=
             this.currentTime) { this.config.loop.end = null;
                      }
                      this.config.loop.start = this.currentTime;
                      // this.config.loop.indicator.start =
             this.elements.display.played.value; break;
                  ...
                  case 'end':
                      ...
                      // this.config.loop.indicator.end =
             this.elements.display.played.value; break;
