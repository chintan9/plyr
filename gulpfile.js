    ...
    gulp.task(
        ...
        () =>
            gulp.src(src)
                ...
                .pipe(header('typeof navigator === "object" && ')) // "Support"
                                                                   // SSR (#935)
                .pipe(
                    rename({
                      extname : `.${extension}`,
...
// Build distribution
gulp.task('build',
          gulp.series(tasks.clean, gulp.parallel(...tasks.js, ...tasks.css,
                                                 ...tasks.sprite)));
...
const canDeploy = () => {
  ...
  if (!allowed.includes(branch.current)) {
    console.error(`Must be on ${allowed.join(', ')} to publish! (current: ${
        branch.current})`);
  ...
  return (gulp.src(paths.upload)
              ...
              .pipe(
                  replace(
                      /sourceMappingURL=([\w-?.]+)/,
                      (match, filename) =>
                          `sourceMappingURL=${filename.replace(minSuffix, '')}`,
                      ),
                  )
              .pipe(size(sizeOptions))
...
gulp.task('open', () => {
  ...
  return gulp.src(__filename)
      .pipe(
          open({
            uri : `https://${domain}/${
                branch.current === branch.beta ? 'beta' : ''}`,
          }),
      );
});
